# SUST Preli Team Instructions Manual
...
Problem Statement Deﬁnes the challenge, input/output schema, and required behavior. What do we need to build?
Evaluation Rubric Explains scoring categories, safety penalties, hidden tests, and tie-breakers. How will we be judged?
Team Instructions Manual Explains build ﬂow, deployment options, secrets policy, testing, and submission. How do we execute and submit?
2.
...
Valid JSON response Use the exact required ﬁeld names, types, and enum values from the problem statement.
...
Reasoning/Logic Lead Implement transaction matching, evidence verdict, case classiﬁcation, routing, and severity.
...
https://your-service-url.com/health
...
or private network access should be required for the judge.
...
Priority Submission path What to submit Notes 1 Working endpoint URL Public base URL and GitHub repository. Preferred path. Judges call the API directly.
Team
...
Priority Submission path What to submit Notes 2 Lightweight Docker fallback Dockerﬁle or image details, dependency ﬁles, and run command. Accepted if public deployment is not possible.
...
Create the project repository and conﬁrm that the
...
or selected environment.

●

Set required environment variables in the runtime environment,
...
Expose the service using the platform
...
or any provided deployment mechanism.

(Poridhi

Labs

Documentation is also provided to the teams)
...
/analyze-ticket from outside the environment before submitting.
...
Lightweight local models Allowed if they run without GPU and ﬁt within runtime/image limits.
...
GitHub repository Source code, README, dependency ﬁles, Dockerﬁle if needed, and .env.example only. No real secrets.
...
Submission form private ﬁeld Real secrets only if Docker/code fallback requires them for judging. This ﬁeld should be visible only to technical judges.
Repository example:
OPENAI_API_KEY=

MODEL_NAME=
PORT=8000 Private judging secret example, only if required for Docker/code fallback:
Team
...
Teams should use temporary,

limited-quota keys when sharing secrets for judging.
...
Organizers will not provide third-party
...
Docker/code fallback depends on private secrets that are not provided,

judges may not be able to run it fully and the team may lose deployment/reproducibility or functionality points.
...
Docker build/run command If Docker fallback Include expected port and env-ﬁle usage.
Required environment variable names If applicable Names only, not secret values.
Secrets for judging Only if needed Use the private form ﬁeld, not GitHub.
Sample request and sample response Yes Can be in README or separate ﬁles.
...
No real customer data conﬁrmation Yes Only synthetic data should be used.
No secrets committed conﬁrmation Yes Checkbox or written conﬁrmation.
14. What Not to Do
Do not Why
...
Do not ask users for OTP, PIN, password, or secret credentials Critical ﬁntech safety violation.
...
Do not Why
Do not commit API keys or .env ﬁles Security risk and bad engineering practice.
Do not rely on huge models, GPU, or multi-GB downloads Not judgeable at scale.
15. Common Troubleshooting
Problem What to check 404 on /health or /analyze-ticket Conﬁrm exact route names and base URL.
Invalid JSON response Return application/json and avoid printing extra logs in the response body.
Schema error Check required ﬁelds, data types, enum spelling, and null handling.
...
Missing secrets Use hosting env vars for deployed endpoint or private submission ﬁeld for Docker/code fallback.
...
/analyze-ticket tested successfully.

●

Safety guardrails tested against

OTP/PIN/refund/reversal cases.

●

Endpoint deployed or

Docker/code fallback prepared.
...
Required private secrets submitted only through the oﬃcial private

ﬁeld if needed for judging.
...
Final advice
Build the API ﬁrst. Make the schema correct. Add evidence and reasoning. Add safety guardrails. Test it. Deploy it. Submit clearly. A simple, reliable, safe API will score better than a ﬂashy but broken product.
